...
void saveseis_glob(FILE *fp, float **sectionvx, float **sectionvy,float **sectionvz,float **sectionp,
        float **sectioncurl, float **sectiondiv, int  **recpos, int  **recpos_loc,int ntr, float ** srcpos, int ishot, int ns,
        int iter, int type_switch)
{
    /* type_switch:
     *  1== synthetic data
    ...
    switch (SEISMO) {
        case 1 : /* particle velocities only */
            if (WAVETYPE==1 || WAVETYPE==3) {
                //if(ADJOINT_TYPE==1) {
                if(ADJOINT_TYPE==1 || FORWARD_ONLY==1) {
