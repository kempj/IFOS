...
int main(int argc, char **argv)
{
    ...
    float ** vpmat;
    float ***forward_prop_x, ***forward_prop_y, ***forward_prop_rho_x, ***forward_prop_u, ***forward_prop_rho_y, ***forward_prop_p;
            ...
            if(FWI_run) {
                ...
                if(!FORWARD_ONLY && MYID==0) {
                    if(iter==1) {
                        ...
                        if(WAVETYPE==3) {
                            FPL2_JOINT=fopen(L2_joint_log,"w");
                        }
                    } else {
                        FPL2=fopen(MISFIT_LOG_FILE,"a");
                        if(WAVETYPE==3) {
                            FPL2_JOINT=fopen(L2_joint_log,"a");
                        }
                    }
                }
                ...
                if(INVTYPE==2) {
                    ...
                    for (ishot=1; ishot<=nshots; ishot+=SHOTINC) {
                        ...
                        if(WAVETYPE==2 || WAVETYPE==3) {
                            SOURCE_SHAPE_SH=SOURCE_SHAPE_OLD_SH;
                        }
                        /*------------------------------------------------------------------------------*/
                        ...
                        if(((INV_STF==1)&&( (iter==1) || (do_stf==1) )) && (gradient_optimization==1) ) {
                            ...
                            if(SOURCE_SHAPE==3) {
                                declare_error("SOURCE_SHAPE==3 isn't possible with INV_STF==1");
                            }
                            MPI_Barrier(MPI_COMM_WORLD);
                            ...
                            if(L) {
                                if(!ACOUSTIC) {
                                    zero_fdveps_visc(-nd+1,NY+nd,-nd+1,NX+nd,pvx,pvy,pvz,psxx,psyy,psxy,psxz,
                                            psyz,ux,uy,uxy,pvxp1,pvyp1,psi_sxx_x,psi_sxy_x,psi_sxz_x,psi_vxx,
                                            psi_vyx,psi_vzx,psi_syy_y,psi_sxy_y,psi_syz_y,psi_vyy,psi_vxy,
                        ...
                        for (int nt=1; nt<=NT; nt++) {
                            ...
                            if((SNAP) && (nt==lsnap) && (nt<=TSNAP2/DT)) {
                                ...
                                if(WAVETYPE==2||WAVETYPE==3) {
                                    snap_SH(FP,nt,nsnap,pvz,pu,ppi,hc,ishot);
                                }
