...
void smooth_model(float ** ppi, float ** pu, float ** prho, int iter)
{
	...
	float **modeltmp_vp, **modeltmps_vp, **modeltmp_vs, **modeltmps_vs, grad, normgauss, smooth_meter;
	...
	for (i=1;i<=NXG;i++){
	   for (j=1;j<=NYG;j++){
		     ...
		     if(i>FILT_SIZE && j>FILT_SIZE && i<=(NXG-FILT_SIZE) && j<=(NYG-FILT_SIZE)){
		     for (i1=(i-FILT_SIZE);i1<=(i+FILT_SIZE);i1++){
	                 for (j1=(j-FILT_SIZE);j1<=(j+FILT_SIZE);j1++){
			 
                         modeltmps_vp[j][i] += modeltmp_vp[j1][i1] * (1/(sqrt(2*PI)*FILT_SIZE*FILT_SIZE)) * exp((-1/2)*((i-i1)*(i-i1)+(j-j1)*(j-j1))/(FILT_SIZE*FILT_SIZE)); 
			 modeltmps_vs[j][i] += modeltmp_vs[j1][i1] * (1/(sqrt(2*PI)*FILT_SIZE*FILT_SIZE)) * exp((-1/2)*((i-i1)*(i-i1)+(j-j1)*(j-j1))/(FILT_SIZE*FILT_SIZE));
			 normgauss += (1/(sqrt(2*PI)*FILT_SIZE*FILT_SIZE)) * exp((-1/2)*((i-i1)*(i-i1)+(j-j1)*(j-j1))/(FILT_SIZE*FILT_SIZE)); 
		     ...
		     if(j<=FILT_SIZE && i<NXG && i>n){
		     for (i1=1;i1<=(i+FILT_SIZE);i1++){
	                 for (j1=1;j1<=(j+FILT_SIZE);j1++){
		     ...
		     if(i<=FILT_SIZE && j<(NYG+FILT_SIZE)){
		     for (i1=1;i1<=(i+FILT_SIZE+FILT_SIZE);i1++){
	                 for (j1=1;j1<=(j+FILT_SIZE);j1++){
		     ...
		     /*
		     if((i-FILT_SIZE)>0 && (j-FILT_SIZE)>0 && i>=NXG && j<NYG){
		     for (i1=(i-FILT_SIZE);i1<=i;i1++){
	                 for (j1=(j-FILT_SIZE);j1<=j;j1++){
		     ...
		     if((i-FILT_SIZE)>0 && (j-FILT_SIZE)>0 && j>=NYG){
		     for (i1=(i-FILT_SIZE);i1<=i;i1++){
	                 for (j1=(j-FILT_SIZE);j1<=j;j1++){
	...
	fprintf(FP,"\n \t ---- V_p and V_s models are smoothed with half filter length of %d gridpoints which is equivalent to %4.2f meter---- \n",FILT_SIZE,smooth_meter);
