...
float  ** srcpos=NULL, **srcpos_loc=NULL, ** srcpos1=NULL, **srcpos_loc_back=NULL, ** signals=NULL, ** signals_rec=NULL, *hc=NULL, ** dsignals=NULL, ** signals_stf=NULL, * signals_stf_temp=NULL;
int   ** recpos=NULL, ** recpos_loc=NULL;
int   ** tracekill=NULL, TRKILL, DTRKILL;
float * temp_TS_x, * temp_TS_y, * temp_conv_x, * temp_conv_y;
...
temp_TS_x =  vector(1,(NT/DTINV));
temp_TS_y =  vector(1,(NT/DTINV));
temp_conv_x =  vector(1,(NT/DTINV));
temp_conv_y =  vector(1,(NT/DTINV));
signals_stf_temp = vector(1,(NT/DTINV));
...
/*------------ convolve source time function with seismograms at each gridpoint --------------*/

for (i=0;i<NT;i++){ signals_stf_temp[i]=signals_stf[ishot][i]; }
...
for (i=1;i<=NX;i=i+IDXI){   
	    for (j=1;j<=NY;j=j+IDYI){ 
			 ...
			 for (nt=1;nt<=NT;nt=nt+DTINV){
			      ...
			      temp_TS_x[h] =  forward_prop_x[imat];
			      temp_TS_y[h] =  forward_prop_y[imat];
			 h++;
	                 }

			 /* convolve time series in frequency domain */
			  conv_FD(temp_TS_x,signals_stf_temp,temp_conv_x,(NT/DTINV));
			  conv_FD(temp_TS_y,signals_stf_temp,temp_conv_y,(NT/DTINV));
			  
			 h=1; 
			 for (nt=1;nt<=NT;nt=nt+DTINV){
			    imat = imat1 + nxnyi*(nt-1);
			    
			    forward_prop_x[imat]=temp_conv_x[h];
			    forward_prop_y[imat]=temp_conv_y[h];
			  h++;
			 }
...
if ((ntr>0) && (SEISMO)){	
 ...
 free_vector(temp_TS_x,1,(NT/DTINV));
 free_vector(temp_TS_y,1,(NT/DTINV));
 free_vector(temp_conv_x,1,(NT/DTINV));
 free_vector(temp_conv_y,1,(NT/DTINV));
 free_vector(signals_stf_temp,1,(NT/DTINV));
