...
int ntr_glob, int ntr,float ** srcpos, int ishot, int ns, int iter, int nshots, float ** signals_stf, float ** signals){ 
	...
	unsigned int nrec, nsamp, i, t, npairs;
	float dt;
	float xr=0.0, yr=0.0;
	float XS=0.0, YS=0.0;
	...
	for (tt=1;tt<=NT;tt++){
		stf_conv_temp[tt]=signals[ishot][tt];	
			        }

	for (i=0; i<npairs; ++i){
	  
		pairs.pairs[i].synthetics=&stf_conv_temp[1];
		pairs.pairs[i].convolvedsynthetics=&stf_conv[1];
	...
	for (i=0;i<NT;i++){ stf_conv[i]=stf_conv_temp[i]*DT*DT; }
