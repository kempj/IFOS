...
void read_par_json(FILE *fp, char *fileinp)
{
    ...
    extern int   NX, NY, FDORDER, MAXRELERROR, SOURCE_SHAPE,SOURCE_SHAPE_SH, SOURCE_TYPE, SNAP,
           SNAP_FORMAT, ACOUSTIC, L, VERBOSE, WAVETYPE,JOINT_INVERSION_PSV_SH_TYPE,JOINT_EQUAL_WEIGHTING;
    ...
    extern int SWS_TAPER_GRAD_VERT, SWS_TAPER_GRAD_HOR, SWS_TAPER_GRAD_SOURCES,
           SWS_TAPER_CIRCULAR_PER_SHOT, SRTSHAPE, FILTSIZE;
    ...
    if (MYID == 0) {
        ...
        if (get_int_from_objectlist("WAVETYPE",number_readobjects,&WAVETYPE,varname_list, value_list)) {
            ...
            if(WAVETYPE==3) {
                if (get_int_from_objectlist("JOINT_EQUAL_WEIGHTING",number_readobjects,&JOINT_EQUAL_WEIGHTING,
                ...
                if (get_int_from_objectlist("JOINT_INVERSION_PSV_SH_TYPE",number_readobjects,
                                            &JOINT_INVERSION_PSV_SH_TYPE,varname_list, value_list)) {
                ...
                if (get_float_from_objectlist("JOINT_INVERSION_PSV_SH_ALPHA_VS",number_readobjects,
                                              &JOINT_INVERSION_PSV_SH_ALPHA_VS,varname_list, value_list)) {
                ...
                if (get_float_from_objectlist("JOINT_INVERSION_PSV_SH_ALPHA_RHO",number_readobjects,
                                              &JOINT_INVERSION_PSV_SH_ALPHA_RHO,varname_list, value_list)) {
        ...
        if (get_int_from_objectlist("SOURCE_SHAPE",number_readobjects,&SOURCE_SHAPE,varname_list,
                                    value_list))
            ...
            if ((SOURCE_SHAPE==3)||(SOURCE_SHAPE==7)||(INV_STF==1)) {
                if (WAVETYPE==1 || WAVETYPE==3) {
                    if (get_string_from_objectlist("SIGNAL_FILE",number_readobjects,SIGNAL_FILE,varname_list,
                                                   value_list))
                        declare_error("Variable SIGNAL_FILE could not be retrieved from the json input file!");
                }
                if (WAVETYPE==2 || WAVETYPE==3) {
                    if (get_string_from_objectlist("SIGNAL_FILE_SH",number_readobjects,SIGNAL_FILE_SH,varname_list,
                                                   value_list))
                        declare_error("Variable SIGNAL_FILE_SH could not be retrieved from the json input file!");
                }
            }
        }
        if (get_int_from_objectlist("SOURCE_SHAPE_SH",number_readobjects,&SOURCE_SHAPE_SH,varname_list,
                                    value_list)) {
            if (WAVETYPE==2 || WAVETYPE==3) {
        ...
        if (SRCREC==1) {
            if (get_string_from_objectlist("SOURCE_FILE",number_readobjects,SOURCE_FILE,varname_list,
                                           value_list))
                declare_error("Variable SOURCE_FILE could not be retrieved from the json input file!");
            if (get_int_from_objectlist("RUN_MULTIPLE_SHOTS",number_readobjects,&RUN_MULTIPLE_SHOTS,
                                        varname_list, value_list))
                declare_error("Variable RUN_MULTIPLE_SHOTS could not be retrieved from the json input file!");
        }
        if (SRCREC==2) {
            if (get_float_from_objectlist("PLANE_WAVE_DEPTH",number_readobjects,&PLANE_WAVE_DEPTH,varname_list,
                                          value_list))
        ...
        if (get_int_from_objectlist("SNAP",number_readobjects,&SNAP,varname_list, value_list)) {
            ...
            if (SNAP>0) {
                ...
                if (get_int_from_objectlist("SNAPSHOT_START",number_readobjects,&SNAPSHOT_START,varname_list,
                                            value_list))
                    declare_error("Variable SNAPSHOT_START could not be retrieved from the json input file!");
                if (get_int_from_objectlist("SNAPSHOT_END",number_readobjects,&SNAPSHOT_END,varname_list,
                                            value_list))
                    declare_error("Variable SNAPSHOT_START could not be retrieved from the json input file!");
                if (get_int_from_objectlist("SNAPSHOT_INCR",number_readobjects,&SNAPSHOT_INCR,varname_list,
                                            value_list))
            ...
            if (SEISMO>0) {
                ...
                if (get_int_from_objectlist("REC_ARRAY",number_readobjects,&REC_ARRAY,varname_list, value_list)) {
                    ...
                    if (REC_ARRAY>0) {
                        ...
                        if (get_float_from_objectlist("REC_ARRAY_DEPTH",number_readobjects,&REC_ARRAY_DEPTH,varname_list,
                                                      value_list))
                            declare_error("Variable REC_ARRAY_DEPTH could not be retrieved from the json input file!");
                        if (get_float_from_objectlist("REC_ARRAY_DIST",number_readobjects,&REC_ARRAY_DIST,varname_list,
                                                      value_list))
        ...
        if (get_int_from_objectlist("L",number_readobjects,&L,varname_list, value_list)) {
            ...
            if (get_float_from_objectlist("F_REF",number_readobjects,&F_REF,varname_list, value_list)) {
                F_REF=-1.0;
                fprintf(fp,
                        "Reference frequency for viscoelastic modeling is set to center frequency of the source wavelet.\n");
            }
        }
        /*=================================
        ...
        if (get_int_from_objectlist("FORWARD_ONLY",number_readobjects,&FORWARD_ONLY,varname_list,value_list)) {
            ...
            if (FORWARD_ONLY==0) {  /* FWI is calculated */
                ...
                if (get_int_from_objectlist("ADJOINT_TYPE",number_readobjects,&ADJOINT_TYPE,varname_list,
                                            value_list))
                    declare_error("Variable ADJOINT_TYPE could not be retrieved from the json input file!");
                if (get_string_from_objectlist("MISFIT_LOG_FILE",number_readobjects,MISFIT_LOG_FILE,varname_list,
                                               value_list)) {
                ...
                if (get_int_from_objectlist("USE_WORKFLOW",number_readobjects,&USE_WORKFLOW,varname_list,
                                            value_list)) {
                    USE_WORKFLOW=0;
                } else {
                    if (get_string_from_objectlist("FILE_WORKFLOW",number_readobjects,FILE_WORKFLOW,varname_list,
                                                   value_list))
                ...
                if (get_int_from_objectlist("EPRECOND",number_readobjects,&EPRECOND,varname_list, value_list)) {
                    ...
                    if (get_int_from_objectlist("EPRECOND_ITER",number_readobjects,&EPRECOND_ITER,varname_list,
                                                value_list)) {
                        EPRECOND_ITER=0;
                        fprintf(fp,"Variable EPRECOND_ITER is set to default value %d.\n",EPRECOND_ITER);
                    }
                    if (get_int_from_objectlist("EPRECOND_PER_SHOT",number_readobjects,&EPRECOND_PER_SHOT,varname_list,
                                                value_list)) {
                        EPRECOND_PER_SHOT=0;
                    ...
                    if (get_int_from_objectlist("EPRECOND_PER_SHOT_SH",number_readobjects,&EPRECOND_PER_SHOT_SH,
                                                varname_list, value_list)) {
                    ...
                    if (get_float_from_objectlist("EPSILON_WE_SH",number_readobjects,&EPSILON_WE_SH,varname_list,
                                                  value_list)) {
                        EPSILON_WE_SH=EPSILON_WE;
                        fprintf(fp,"Variable EPSILON_WE_SH is set to EPSILON_WE=%f.\n",EPSILON_WE_SH);
                    }
                }
                if (get_int_from_objectlist("TESTSHOT_START",number_readobjects,&TESTSHOT_START,varname_list,
                                            value_list))
                    declare_error("Variable TESTSHOT_START could not be retrieved from the json input file!");
                if (get_int_from_objectlist("TESTSHOT_END",number_readobjects,&TESTSHOT_END,varname_list,
                                            value_list))
                    declare_error("Variable TESTSHOT_START could not be retrieved from the json input file!");
                if (get_int_from_objectlist("TESTSHOT_INCR",number_readobjects,&TESTSHOT_INCR,varname_list,
                                            value_list))
                    declare_error("Variable TESTSHOT_INCR could not be retrieved from the json input file!");
                NO_OF_TESTSHOTS=(TESTSHOT_END-TESTSHOT_START)/TESTSHOT_INCR
                                +1;  /* calculation of number of testsshots */
                /* Cosine taper */
                ...
                if (get_int_from_objectlist("TAPERLENGTH",number_readobjects,&TAPERLENGTH,varname_list,
                                            value_list)) {
                    TAPERLENGTH=1000;
                    fprintf(fp,"Variable TAPERLENGTH is set to default value %d.\n",TAPERLENGTH);
                }
                /* Definition of gradient taper geometry */
                if (get_int_from_objectlist("SWS_TAPER_GRAD_VERT",number_readobjects,&SWS_TAPER_GRAD_VERT,
                                            varname_list, value_list)) {
                    SWS_TAPER_GRAD_VERT=0;
                    fprintf(fp,"Variable SWS_TAPER_GRAD_VERT is set to default value %d.\n",SWS_TAPER_GRAD_VERT);
                }
                if (get_int_from_objectlist("SWS_TAPER_GRAD_HOR",number_readobjects,&SWS_TAPER_GRAD_HOR,
                                            varname_list, value_list)) {
                ...
                if (get_int_from_objectlist("SWS_TAPER_GRAD_SOURCES",number_readobjects,&SWS_TAPER_GRAD_SOURCES,
                                            varname_list, value_list)) {
                    SWS_TAPER_GRAD_SOURCES=0;
                    fprintf(fp,"Variable SWS_TAPER_GRAD_SOURCES is set to default value %d.\n",SWS_TAPER_GRAD_SOURCES);
                }
                if (get_int_from_objectlist("SWS_TAPER_CIRCULAR_PER_SHOT",number_readobjects,
                                            &SWS_TAPER_CIRCULAR_PER_SHOT,varname_list, value_list)) {
                ...
                if (get_int_from_objectlist("SWS_TAPER_FILE",number_readobjects,&SWS_TAPER_FILE,varname_list,
                                            value_list)) {
                    SWS_TAPER_FILE=0;
                    fprintf(fp,"Variable SWS_TAPER_FILE is set to default value %d.\n",SWS_TAPER_FILE);
                }
                if (SWS_TAPER_FILE==1) {
                    if (get_string_from_objectlist("TAPER_FILE_NAME",number_readobjects,TAPER_FILE_NAME,varname_list,
                                                   value_list))
                        declare_error("Variable TAPER_FILE_NAME could not be retrieved from the json input file!");
                }
                if (get_int_from_objectlist("SWS_TAPER_FILE_PER_SHOT",number_readobjects,&SWS_TAPER_FILE_PER_SHOT,
                                            varname_list, value_list)) {
                ...
                if (get_int_from_objectlist("SPATFILTER",number_readobjects,&SPATFILTER,varname_list, value_list)) {
                    ...
                    if (SPATFILTER==1) {
                        if (get_int_from_objectlist("SPAT_FILT_SIZE",number_readobjects,&SPAT_FILT_SIZE,varname_list,
                                                    value_list))
                        ...
                        if (get_int_from_objectlist("SPAT_FILT_ITER",number_readobjects,&SPAT_FILT_ITER,varname_list,
                                                    value_list))
                ...
                if (get_int_from_objectlist("GRAD_FILTER",number_readobjects,&GRAD_FILTER,varname_list,
                                            value_list)) {
                    GRAD_FILTER=0;
                    fprintf(fp,"Variable GRAD_FILTER is set to default value %d.\n",GRAD_FILTER);
                }
                if (get_int_from_objectlist("FILT_SIZE_GRAD",number_readobjects,&FILT_SIZE_GRAD,varname_list,
                                            value_list)) {
                    FILT_SIZE_GRAD=0;
                    fprintf(fp,"Variable FILT_SIZE_GRAD is set to default value %d.\n",FILT_SIZE_GRAD);
                }
                if (get_int_from_objectlist("GRAD_FILT_WAVELENGTH",number_readobjects,&GRAD_FILT_WAVELENGTH,
                                            varname_list, value_list)) {
                ...
                if (get_string_from_objectlist("INV_MODELFILE",number_readobjects,INV_MODELFILE,varname_list,
                                               value_list))
                ...
                if (get_int_from_objectlist("INV_RHO_ITER",number_readobjects,&INV_RHO_ITER,varname_list,
                                            value_list)) {
                    INV_RHO_ITER=0;
                    fprintf(fp,"Variable INV_RHO_ITER is set to default value %d.\n",INV_RHO_ITER);
                }
                /* Inversion for Vp */
                if (get_int_from_objectlist("INV_VP_ITER",number_readobjects,&INV_VP_ITER,varname_list,
                                            value_list)) {
                    INV_VP_ITER=0;
                    fprintf(fp,"Variable INV_VP_ITER is set to default value %d.\n",INV_VP_ITER);
                }
                /* Inversion for Vs */
                if (get_int_from_objectlist("INV_VS_ITER",number_readobjects,&INV_VS_ITER,varname_list,
                                            value_list)) {
                    INV_VS_ITER=0;
                    fprintf(fp,"Variable INV_VS_ITER is set to default value %d.\n",INV_VS_ITER);
                }
                /* Vp/Vs-Ratio */
                if (get_float_from_objectlist("VP_VS_RATIO",number_readobjects,&VP_VS_RATIO,varname_list,
                                              value_list)) {
                    VP_VS_RATIO=0.0;
                    fprintf(fp,
                            "Variable VP_VS_RATIO is set to default value %4.2f which means that it is disregarded.\n",
                            VP_VS_RATIO);
                }
                ...
                if (get_float_from_objectlist("VPUPPERLIM",number_readobjects,&VPUPPERLIM,varname_list,
                                              value_list)) {
                    VPUPPERLIM=25000.0;
                    fprintf(fp,"Variable VPUPPERLIM is set to default value %f.\n",VPUPPERLIM);
                }
                if (get_float_from_objectlist("VPLOWERLIM",number_readobjects,&VPLOWERLIM,varname_list,
                                              value_list)) {
                    VPLOWERLIM=0.0;
                    fprintf(fp,"Variable VPLOWERLIM is set to default value %f.\n",VPLOWERLIM);
                }
                if (get_float_from_objectlist("VSUPPERLIM",number_readobjects,&VSUPPERLIM,varname_list,
                                              value_list)) {
                    VSUPPERLIM=25000.0;
                    fprintf(fp,"Variable VSUPPERLIM is set to default value %f.\n",VSUPPERLIM);
                }
                if (get_float_from_objectlist("VSLOWERLIM",number_readobjects,&VSLOWERLIM,varname_list,
                                              value_list)) {
                    VSLOWERLIM=0.0;
                    fprintf(fp,"Variable VSLOWERLIM is set to default value %f.\n",VSLOWERLIM);
                }
                if (get_float_from_objectlist("RHOUPPERLIM",number_readobjects,&RHOUPPERLIM,varname_list,
                                              value_list)) {
                    RHOUPPERLIM=25000.0;
                    fprintf(fp,"Variable RHOUPPERLIM is set to default value %f.\n",RHOUPPERLIM);
                }
                if (get_float_from_objectlist("RHOLOWERLIM",number_readobjects,&RHOLOWERLIM,varname_list,
                                              value_list)) {
                    RHOLOWERLIM=0.0;
                ...
                if (get_int_from_objectlist("GRAD_METHOD",number_readobjects,&GRAD_METHOD,varname_list, value_list))
                    ...
                    if(GRAD_METHOD==2) {
                        if (get_int_from_objectlist("LBFGS_STEP_LENGTH",number_readobjects,&LBFGS_STEP_LENGTH,varname_list,
                                                    value_list)) {
                            LBFGS_STEP_LENGTH=1;
                        ...
                        if (get_float_from_objectlist("LBFGS_SCALE_GRADIENTS",number_readobjects,&LBFGS_SCALE_GRADIENTS,
                                                      varname_list, value_list)) {
                            LBFGS_SCALE_GRADIENTS=1;
                        }
                        if (get_int_from_objectlist("WOLFE_CONDITION",number_readobjects,&WOLFE_CONDITION,varname_list,
                                                    value_list)) {
                            WOLFE_CONDITION=1;
                            fprintf(fp,"Variable WOLFE_CONDITION is set to default value %d.\n",WOLFE_CONDITION);
                        } else {
                            if (get_int_from_objectlist("WOLFE_NUM_TEST",number_readobjects,&WOLFE_NUM_TEST,varname_list,
                                                        value_list)) {
                                WOLFE_NUM_TEST=10;
                                fprintf(fp,"Variable WOLFE_NUM_TEST is set to default value %d.\n",WOLFE_NUM_TEST);
                            }
                            if (get_int_from_objectlist("WOLFE_TRY_OLD_STEPLENGTH",number_readobjects,&WOLFE_TRY_OLD_STEPLENGTH,
                                                        varname_list, value_list)) {
                                WOLFE_TRY_OLD_STEPLENGTH=0;
                                fprintf(fp,"Variable WOLFE_TRY_OLD_STEPLENGTH is set to default value %d.\n",
                                        WOLFE_TRY_OLD_STEPLENGTH);
                            }
                            if (get_float_from_objectlist("WOLFE_C1_SL",number_readobjects,&WOLFE_C1_SL,varname_list,
                                                          value_list)) {
                                WOLFE_C1_SL=1e-4;
                                fprintf(fp,"Variable WOLFE_C1_SL is set to default value %f.\n",WOLFE_C1_SL);
                            }
                            if (get_float_from_objectlist("WOLFE_C2_SL",number_readobjects,&WOLFE_C2_SL,varname_list,
                                                          value_list)) {
                                WOLFE_C2_SL=0.9;
                ...
                if (get_int_from_objectlist("MODEL_FILTER",number_readobjects,&MODEL_FILTER,varname_list,
                                            value_list)) {
                    MODEL_FILTER=0;
                ...
                if(INV_STF) {
                    ...
                    if (get_int_from_objectlist("TRKILL_STF",number_readobjects,&TRKILL_STF,varname_list, value_list)) {
                        ...
                        if (TRKILL_STF==1) {
                            if (get_int_from_objectlist("TRKILL_STF_OFFSET",number_readobjects,&TRKILL_STF_OFFSET,varname_list,
                                                        value_list)) {
                                TRKILL_STF_OFFSET=0;
                                if (get_string_from_objectlist("TRKILL_FILE_STF",number_readobjects,TRKILL_FILE_STF,varname_list,
                                                               value_list))
                                ...
                                if(TRKILL_STF_OFFSET==1) { /* Only Offset based TRKill */
                                    if (get_int_from_objectlist("TRKILL_STF_OFFSET_INVERT",number_readobjects,&TRKILL_STF_OFFSET_INVERT,
                                                                varname_list, value_list)) {
                                        TRKILL_STF_OFFSET_INVERT=0;
                                    }
                                    if (get_float_from_objectlist("TRKILL_STF_OFFSET_LOWER",number_readobjects,&TRKILL_STF_OFFSET_LOWER,
                                                                  varname_list, value_list)) {
                                        TRKILL_STF_OFFSET_LOWER=0.0;
                                    }
                                    if (get_float_from_objectlist("TRKILL_STF_OFFSET_UPPER",number_readobjects,&TRKILL_STF_OFFSET_UPPER,
                                                                  varname_list, value_list)) {
                ...
                if (get_int_from_objectlist("TIME_FILT",number_readobjects,&TIME_FILT,varname_list, value_list)) {
                    ...
                    if (get_int_from_objectlist("WRITE_FILTERED_DATA",number_readobjects,&WRITE_FILTERED_DATA,
                                                varname_list, value_list)) {
                        WRITE_FILTERED_DATA=0;
                    }
                    if (get_float_from_objectlist("F_HIGH_PASS",number_readobjects,&F_HIGH_PASS,varname_list,
                                                  value_list)) {
                    ...
                    if (TIME_FILT==1) {
                        if (get_float_from_objectlist("F_LOW_PASS_START",number_readobjects,&F_LOW_PASS_START,varname_list,
                                                      value_list)) {
                            /* Support of old variable naming: Test if old variable naming is used */
                            if (get_float_from_objectlist("FC_START",number_readobjects,&F_LOW_PASS_START,varname_list,
                                                          value_list)) {
                                declare_error("Variable F_LOW_PASS_START could not be retrieved from the json input file!");
                            }
                        }
                        if (get_float_from_objectlist("F_LOW_PASS_END",number_readobjects,&F_LOW_PASS_END,varname_list,
                                                      value_list)) {
                            /* Support of old variable naming: Test if old variable naming is used */
                            if (get_float_from_objectlist("FC_END",number_readobjects,&F_LOW_PASS_END,varname_list,
                                                          value_list)) {
                                declare_error("Variable F_LOW_PASS_END could not be retrieved from the json input file!");
                            }
                        }
                        if (get_float_from_objectlist("F_LOW_PASS_INCR",number_readobjects,&F_LOW_PASS_INCR,varname_list,
                                                      value_list)) {
                            /* Support of old variable naming: Test if old variable naming is used */
                            if (get_float_from_objectlist("FC_INCR",number_readobjects,&F_LOW_PASS_INCR,varname_list,
                                                          value_list)) {
                ...
                if (LNORM==8) {
                    if (get_float_from_objectlist("WATERLEVEL_LNORM8",number_readobjects,&WATERLEVEL_LNORM8,
                                                  varname_list, value_list))
                ...
                if (get_int_from_objectlist("TRKILL",number_readobjects,&TRKILL,varname_list, value_list)) {
                    ...
                    if (TRKILL==1) {
                        if (get_int_from_objectlist("TRKILL_OFFSET",number_readobjects,&TRKILL_OFFSET,varname_list,
                                                    value_list)) {
                            TRKILL_OFFSET=0;
                            if (get_string_from_objectlist("TRKILL_FILE",number_readobjects,TRKILL_FILE,varname_list,
                                                           value_list))
                            ...
                            if(TRKILL_OFFSET==1) { /* Only Offset based TRKill */
                                if (get_float_from_objectlist("TRKILL_OFFSET_LOWER",number_readobjects,&TRKILL_OFFSET_LOWER,
                                                              varname_list, value_list)) {
                                    TRKILL_OFFSET_LOWER=0.0;
                                }
                                if (get_float_from_objectlist("TRKILL_OFFSET_UPPER",number_readobjects,&TRKILL_OFFSET_UPPER,
                                                              varname_list, value_list)) {
                ...
                if (get_int_from_objectlist("TIMEWIN",number_readobjects,&TIMEWIN,varname_list, value_list)) {
                    ...
                    if (TIMEWIN==1) {
                        if (get_int_from_objectlist("TW_IND",number_readobjects,&TW_IND,varname_list, value_list)) {
                            ...
                            if (TW_IND>2)
                                declare_error("Only TW_IND=1 (one time window) and TW_IND=2 (two time windows) possible");
                        }
                        ...
                        if (get_float_from_objectlist("TWLENGTH_PLUS",number_readobjects,&TWLENGTH_PLUS,varname_list,
                                                      value_list))
                            declare_error("Variable TWLENGTH_PLUS could not be retrieved from the json input file!");
                        if (get_float_from_objectlist("TWLENGTH_MINUS",number_readobjects,&TWLENGTH_MINUS,varname_list,
                                                      value_list))
