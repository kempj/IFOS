...
void LBFGS1(float ** waveconv, float ** taper_coeff, int nsrc, float ** srcpos, int ** recpos, int ntr_glob, int iter, float C_vp, float ** gradp, int nfstart_jac,
	...
	float gamma_LBFGS_vp, gamma_LBFGS_vs, gamma_LBFGS_rho, beta_LBFGS_vp, beta_LBFGS_vs, beta_LBFGS_rho, Vp_sum, Vs_sum;
...
if(iter>1){
   ...
   /*printf("k = %d \t iter1 = %d \t MYID = %d \n",k,iter1, MYID);*/
     ...
     for (i=1;i<=NX;i=i+IDX){
       for (j=1;j<=NY;j=j+IDY){
	      ...
	      fread(&modellastiter,sizeof(float),1,FP7);
          s_LBFGS_rho[j][i][iter1] = prho[j][i]-modellastiter;
   ...
   /*printf("k = %d \t iter1 = %d \t MYID = %d \n",k,iter1, MYID);*/
	 ...
	 for(k=iter1;k>=1;k--){
	   ...
	   for (i=1;i<=NX;i=i+IDX){
           for (j=1;j<=NY;j=j+IDY){
              ...
              LBFGSTMP  += y_LBFGS_vp[j][i][k] * s_LBFGS_vp[j][i][k]; 
	          LBFGSTMP1 += y_LBFGS_vs[j][i][k] * s_LBFGS_vs[j][i][k];
	   ...
	   for (i=1;i<=NX;i=i+IDX){
		  ...
		  /* calculate alpha */
		  LBFGSTMP += s_LBFGS_vp[j][i][k]*waveconv[j][i];  
		  LBFGSTMP1 += s_LBFGS_vs[j][i][k]*waveconv_u[j][i];
     ...
     Vp_sum = 0.0;
     MPI_Allreduce(&LBFGSTMP,&Vp_sum,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
     alpha_LBFGS_vp[k] = rho_LBFGS_vp[k] * Vp_sum;
     ...
     alpha_LBFGS_vs[k] = rho_LBFGS_vs[k] * Vp_sum;
	  ...
	  for (i=1;i<=NX;i=i+IDX){
		  ...
		  /* update gradient */
		  waveconv[j][i] = waveconv[j][i] - alpha_LBFGS_vp[k] * y_LBFGS_vp[j][i][k];
		  waveconv_u[j][i] = waveconv_u[j][i] - alpha_LBFGS_vs[k] * y_LBFGS_vs[j][i][k];
     ...
     for (i=1;i<=NX;i=i+IDX){
       for (j=1;j<=NY;j=j+IDY){
          ...
          /* calculate nominator and denominator of gamma */
	  	  LBFGSTMP += y_LBFGS_vp[j][i][iter1] * s_LBFGS_vp[j][i][iter1];
		  LBFGSTMP1 += y_LBFGS_vs[j][i][iter1] * s_LBFGS_vs[j][i][iter1];
		  /*LBFGSTMP += y_LBFGS_rho[k] * s_LBFGS_rho[k];*/  /*XX no density update !!! */

		  LBFGSTMP2 += y_LBFGS_vp[j][i][iter1] * y_LBFGS_vp[j][i][iter1];  
		  LBFGSTMP3 += y_LBFGS_vs[j][i][iter1] * y_LBFGS_vs[j][i][iter1];
	 ...
	 /* Sum nominator and denominator of gamma of all CPUs */
	     ...
	     Vs_sum = 0.0;
         MPI_Allreduce(&LBFGSTMP2,&Vs_sum,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);

	 gamma_LBFGS_vp = Vp_sum/Vs_sum;
     ...
     /* Sum nominator and denominator of gamma of all CPUs */
	     ...
	     Vs_sum = 0.0;
         MPI_Allreduce(&LBFGSTMP3,&Vs_sum,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
	 ...
	 for (i=1;i<=NX;i=i+IDX){
           for (j=1;j<=NY;j=j+IDY){
	           waveconv[j][i] = gamma_LBFGS_vp * waveconv[j][i];
	           waveconv_u[j][i] = gamma_LBFGS_vs * waveconv_u[j][i];
	 ...
	 for(k=1;k<=iter1;k++){
	   ...
	   for (i=1;i<=NX;i=i+IDX){
		  ...
		  /* calculate beta */
		  LBFGSTMP += y_LBFGS_vp[j][i][k]*waveconv[j][i];
		  LBFGSTMP1 += y_LBFGS_vs[j][i][k]*waveconv_u[j][i];
     ...
     Vp_sum = 0.0;
     MPI_Allreduce(&LBFGSTMP,&Vp_sum,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
     beta_LBFGS_vp = rho_LBFGS_vp[k] * Vp_sum;

	 /* Sum over beta of all CPUs */
     Vp_sum = 0.0;
     MPI_Allreduce(&LBFGSTMP1,&Vp_sum,1,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);
     beta_LBFGS_vs = rho_LBFGS_vs[k] * Vp_sum;

	  for (i=1;i<=NX;i=i+IDX){
           for (j=1;j<=NY;j=j+IDY){
               
		  /* update gradient */
		  waveconv[j][i] = waveconv[j][i] + s_LBFGS_vp[j][i][k] * (alpha_LBFGS_vp[k]-beta_LBFGS_vp);  
		  waveconv_u[j][i] = waveconv_u[j][i] + s_LBFGS_vs[j][i][k] * (alpha_LBFGS_vs[k]-beta_LBFGS_vs);
